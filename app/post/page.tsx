...
export default function MyPage() {
  ...
  return (
    // ▼ 背景を画像に変更
    <main
  className="min-h-screen flex items-start justify-center p-6 bg-cover bg-center"
  style={{ backgroundImage: "url('/images/mypage-bg.jpg')" }}
>

      <div className="w-full max-w-3xl">
        <h1 className="mb-4 text-2xl font-bold text-neutral-900">投稿</h1>

        <form
  onSubmit={onSubmit}
  className="space-y-3 rounded-2xl border border-black/10 bg-[#]c0c0c0 p-6 shadow-xl backdrop-blur"
>
  {/* 種類 */}
  <div>
    <label className="block text-sm font-medium">種類</label>
    <select
      className="mt-1 w-full rounded border p-2 bg-white"   // ← bg-white を追加
      value={form.species}
    ...
    {form.species === "その他" && (
      <input
        className="mt-2 w-full rounded border p-2 bg-white"  // ← bg-white を追加
